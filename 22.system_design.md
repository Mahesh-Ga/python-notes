...
[System Design Interview](https://github.com/karanpratapsingh/system-design#system-design-interviews)
...
- A load balancer sits between clients and your servers, deciding which server gets each requests
- Layer determines how much of the traffic the load balancer can actually see and understand.
![alt text](images/Picture2-3.png)

- **Layer 4 (Transport Layer)**
What it sees: Only the packet header — source/destination IP and port. It does not open or read the actual data.
- **Layer 7 (Application Layer)**
What it sees: The full HTTP request — URL path, headers, cookies, method, body.
How it routes:
- `example.com/api/*` → API servers
- `example.com/images/*` → static file servers
- `Cookie: session=abc` → the server holding that user's session
- `/checkout → a separate` → hardened checkout service

| | Layer 4 | Layer 7 |
|---|---|---|
| Sees | IP + port | Full HTTP request |
| Routing basis | Connection | URL, headers, cookies |
| Speed | Very fast | Slower (parses data) |
| Protocols | Any TCP/UDP | HTTP(S), gRPC, etc. |
| TLS | Passes through | Often terminates |
| Smart features | Minimal | Rich (retries, caching, rewrites) |
| Typical use | Raw throughput, DBs, gaming | Web apps, microservices |
...
- TLS is the cryptographic protocol that encrypts data sent over a network. 
- It's what turns `http://` into `https://`. 
- When you see the padlock 🔒 in your browser, that's TLS at work.
- It's the modern successor to SSL (Secure Sockets Layer) — people still say "SSL" out of habit, but SSL is deprecated. What everyone calls "SSL certificates" are really TLS certificates.

#### What TLS Actually Does
It provides three things between two parties (e.g., your browser and a server):
...
3. Integrity — nobody can tamper with the data in transit

#### How It Works (Simplified)
1. **Handshake** — Client and server agree on encryption methods and exchange keys
- Client and server greet each other, agree on TLS version and cipher suite
- Exchange random values and use ECDHE/RSA to derive shared session keys
- Both confirm with "Finished" messages that nothing was tampered with
2. **Certificate check**— Client verifies the server's certificate against a trusted authority (like Let's Encrypt, DigiCert)
- Server sends its X.509 certificate (public key, domain, issuer)
- Client verifies it against a trusted CA (Let's Encrypt, DigiCert) via chain of trust
- Checks domain match, expiry date, and revocation (OCSP/CRL)

3. **Secure channel** — All further data is encrypted

- **TLS**	The encryption protocol behind HTTPS
- **TLS termination**	LB decrypts traffic to inspect it (L7 behavior)
- **TLS pass-through**	LB forwards encrypted traffic untouched (L4 behavior)

HTTPS = HTTP + TLS. It's the same HTTP you know (GET, POST, headers, status codes), but every byte is wrapped inside an encrypted TLS tunnel.

Before any of this works, the server needs a **TLS certificate**.
- A certificate binds a domain name (e.g., example.com) to a public key
- It's digitally signed by a Certificate Authority (CA) — e.g., Let's Encrypt, DigiCert, Google Trust Services
- Your browser/OS ships with a trust store — a list of CAs it trusts.

If a server presents a certificate signed by a trusted CA, and the domain matches, the browser trusts it.

It uses a clever combination of:
- **Asymmetric encryption (RSA/ECDHE)**→ for the initial handshake (slow but secure)

```
Client generates:  private key a, public key A = a*G
Server generates:  private key b, public key B = b*G

Client sends A  ────────────────>  Server
Client receives <────────────────  B

Client computes: SharedSecret = a*B = a*b*G
Server computes: SharedSecret = b*A = b*a*G

Both get the SAME value! 🎉
```
- **Symmetric encryption (AES)** → for actual data transfer (fast)
```
Session Key = HKDF(SharedSecret, ClientRandom, ServerRandom)
            = "K3y$eCr3t!2024..." (256-bit AES key)
```
Now both have the same symmetric key — and it was never sent over the wire!

![HTTPS](images/deepseek_mermaid_20260911_dad307.svg)
### 1.4 Caching
...
- **Range-based** — partition by a value range (e.g. user IDs 1–1M on shard A). Simple, but can create hot shards(where one shard receives a disproportionate share of traffic or load, while other shards remain underutilized).
...
- **Consistency (C)** — every read gets the most recent write or an error. (monetary data)
...
- CP (Bank)	"Sorry, we can't confirm today's prices — we're closed."
- AP (Social feed)	"Here's yesterday's menu — we'll update it when we can."
...
```
Hash(key) % N -- N: Total number of nodes.
```
...
![ch](images/consistent-hashing.webp)

| Traditional hashing | Consistent hashing |
|---|---|
| `hash(key) % N` | Hash key onto ring |
| Depends directly on N | Does not use number of nodes in key mapping |
| Adding server changes N | Adding server adds a point to ring |
| Many keys can move | Only a portion of keys move |
| Removing server also causes many changes | Only that server's keys move to its next neighbor |
