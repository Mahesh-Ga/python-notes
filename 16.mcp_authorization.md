...
Client                         MCP Server                 Authorization Server
  ...
  |--- fetch AS metadata (RFC 8414 / OIDC Discovery) -------------->|
  |<----------------------------------------------- AS metadata ---|
  |--- register client (Client ID Metadata Doc / Dynamic Reg) ------>|
  |--- open browser: /authorize?resource=...&code_challenge=... --->|
  |                                                    user consents |
  |<--------------------------- redirect: code + iss ---------------|
  |--- validate iss, then POST /token (code_verifier, resource) --->|
  |<--------------------------------------------- access token -----|
  |--- MCP request: Authorization: Bearer <token> --->|
  |<---------------------------------- MCP response --|
```

1. **Unauthorized request** — the client calls the MCP server without a token. The server replies `401 Unauthorized` with a `WWW-Authenticate` header pointing to its Protected Resource Metadata URL.
2. **Resource discovery** — the client fetches that metadata document and learns which authorization server protects this MCP server.
3. **Authorization server discovery** — the client fetches the authorization server's own metadata (RFC 8414 or OpenID Connect Discovery) to learn its endpoints and capabilities.
4. **Client registration** — the client obtains a `client_id`, via a Client ID Metadata Document, Dynamic Client Registration, or a pre-registered ID.
5. **Authorization request** — the client generates PKCE parameters, selects the appropriate scopes, records the *expected* issuer, and opens the browser to the authorization server's `/authorize` endpoint — including a `resource` parameter that identifies the target MCP server.
6. **User consent** — the user authenticates and approves the request at the authorization server.
7. **Callback & validation** — the authorization server redirects back with an authorization code (and, per RFC 9207, an `iss` value). The client **must** validate that `iss` matches the issuer it recorded in step 3, to defend against mix-up attacks — and must not act on the response at all if this check fails.
8. **Token exchange** — the client exchanges the code (with the PKCE `code_verifier` and the `resource` parameter) for an access token, and optionally a refresh token.
9. **Authenticated MCP requests** — the client attaches the token as `Authorization: Bearer <token>` on every subsequent request to the MCP server.
