...
### Pessimistic Locking
- Assumes conflicts are likely, so it locks the resource before reading/modifying it — other transactions must wait.
- Prevents conflicts entirely, but reduces concurrency and can cause deadlocks or blocking.
- Typical use: SELECT ... FOR UPDATE in SQL.

BEGIN;
SELECT * FROM accounts WHERE id = 1 FOR UPDATE;  -- locks the row
UPDATE accounts SET balance = balance - 100 WHERE id = 1;
COMMIT;
Any other transaction trying to read/write that row with FOR UPDATE blocks until this one commits/rolls back.

#### Optimistic Locking
- Assumes conflicts are rare, so it allows concurrent reads/writes without locking, then checks for conflicts before committing — usually via a version number or timestamp.
- If the version has changed since it was read, the update is rejected and must be retried.
- Better throughput under low contention, but requires retry logic.

-- Read
SELECT id, balance, version FROM accounts WHERE id = 1;
-- version = 5

-- Update, checking version hasn't changed
UPDATE accounts
...
├──────────────┼────────────────────────────────────────────────────────┼──────────────────────────────────────┤
│ Mechanism    │ Lock acquired upfront                                  │ Version/timestamp check at commit    │
├──────────────┼────────────────────────────────────────────────────────┼──────────────────────────────────────┤
│ Concurrency  │ Lower (blocking)                                       │ Higher (no blocking)                 │
├──────────────┼────────────────────────────────────────────────────────┼──────────────────────────────────────┤
│ Failure mode │ Deadlocks, waiting                                     │ Retry on conflict                    │
├──────────────┼────────────────────────────────────────────────────────┼──────────────────────────────────────┤
│ Best for     │ High-contention writes (e.g., inventory/bank balances) │ Low-contention, read-heavy workloads │
└──────────────┴────────────────────────────────────────────────────────┴──────────────────────────────────────┘99
```
